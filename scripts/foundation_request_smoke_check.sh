...
# RU: Здесь тоже нужен свободный порт, иначе прошлый незавершённый smoke оставляет request verify в ложном bind-conflict.
...
for _ in $(seq 1 30); do
  if ! kill -0 "$API_PID" >/dev/null 2>&1; then
    echo "[request-smoke] foundation API exited before health/live became ready. See /tmp/magon-foundation-request-api.log" >&2
    exit 1
  fi
...
if ! curl -fsS --max-time 5 "$BASE_URL/health/live" >/dev/null 2>&1; then
  echo "[request-smoke] foundation API failed to become ready on $BASE_URL/health/live. See /tmp/magon-foundation-request-api.log" >&2
  exit 1
fi
