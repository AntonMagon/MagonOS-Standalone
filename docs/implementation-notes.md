...
47. Для следующего продуктового прохода каноном считается не только `gpt_doc/codex_wave1_spec_ru.docx`, но и расширенный пакет `platform_documentation_pack_ru_v3` + marketing research. Это значит:
   - публичный фронт обязан продавать управляемый сервис, а не "платформу/маркетплейс/ИИ";
   - главный пользовательский маршрут должен читаться как `витрина/заявка -> квалификация -> предложение -> заказ`, без внутреннего архитектурного жаргона в лоб;
   - operator/admin surfaces должны объяснять действие, статус и следующий шаг, а не показывать сырой API-подобный срез.
48. Маркетинговый слой первой волны должен следовать зафиксированному офферу: `Печать и упаковка без хаоса` и `Одна заявка -> нормальный бриф -> быстрое предложение -> контроль файлов, сроков и исполнителя`. Формулировки уровня `операционный контур`, `первая волна`, `RFQ`, `draft`, `ingest` допустимы только там, где они либо расшифрованы, либо действительно нужны внутренней роли.
