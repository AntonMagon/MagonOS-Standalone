...
- `playwright-interactive`
  Постоянная браузерная сессия для длинных UI-разборов. Полезно, когда нужно не переоткрывать браузер после каждого изменения. Важно: этому skill нужен новый сеанс Codex с включённым `js_repl`.
- `screenshot`
